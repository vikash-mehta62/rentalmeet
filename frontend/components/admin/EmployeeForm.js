              ...
              <div className="border-t pt-5">
                ...
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className={labelCls}>Account Number <span className="text-red-500">*</span></label><input type="text" inputMode="numeric" name="accountNumber" value={formData.accountNumber} onChange={e => handleInputChange({target:{name:'accountNumber',value:e.target.value.replace(/\D/g,'').slice(0,18)}})} className={inputCls} placeholder="Numeric only" maxLength={18} required /></div>
                    <div><label className={labelCls}>IFSC Code <span className="text-red-500">*</span></label><input type="text" name="ifscCode" value={formData.ifscCode} onChange={e => handleInputChange({target:{name:'ifscCode',value:e.target.value.replace(/[^A-Za-z0-9]/g,'').slice(0,11).toUpperCase()}})} placeholder="e.g., SBIN0001234" className={`${inputCls} uppercase`} maxLength={11} required /></div>
