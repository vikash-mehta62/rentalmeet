...
export default function CustomerLayout({ children, activePage = 'dashboard', fullWidth = false, title, subtitle }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 bg-white border-r border-gray-200 flex flex-col transition-all duration-300
        ...
        <div className={`flex items-center border-b border-gray-100 px-4 py-4 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!isCollapsed && (
            <div>
              ...
              <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider mt-0.5">Customer Portal</p>
            </div>
          )}
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const active = resolvedActivePage === item.id;
            return (
      ...
      {/* Main */}
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${isCollapsed ? 'lg:ml-16' : 'lg:ml-56'}`}>
        {/* Top bar */}
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 sticky top-0 z-20">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-gray-900 truncate">{title || activeItemLabel}</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex flex-col text-right leading-tight">
              ...
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Customer</span>
            </div>
