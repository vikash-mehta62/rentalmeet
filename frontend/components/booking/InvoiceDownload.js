...
export default function InvoiceDownload({ booking, userRole = 'customer' }) {
  ...
  const commonCSS = `
    *{margin:0;padding:0;box-sizing:border-box}
    body,div{font-family:'Segoe UI',Arial,sans-serif;font-size:10px;color:#1f2937;-webkit-print-color-adjust:exact}
    .wrap{width:794px;background:#fff;padding:28px 32px}
    ...
    .inv-title{font-size:14px;font-weight:900;letter-spacing:1px;margin-bottom:2px}
    .inv-no{font-size:10px;color:#6b7280;margin-bottom:1px}
    .badge{display:inline-block;padding:2px 8px;border-radius:10px;font-size:9px;font-weight:700;letter-spacing:.5px;margin-top:3px}
    .badge-paid{background:#d1fae5;color:#065f46}
    .badge-pending{background:#fef3c7;color:#92400e}
    .badge-refunded{background:#e0e7ff;color:#3730a3}
    .divider{height:3px;border-radius:2px;margin-bottom:12px}
    .two-col{display:flex;gap:10px;margin-bottom:12px}
    .card{flex:1;border-radius:8px;padding:10px;border:1px solid}
    .card-title{font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:7px;padding-bottom:5px;border-bottom:1px solid}
    .kv{display:flex;gap:4px;margin-bottom:3px;font-size:10px;line-height:1.4}
    .kv-label{color:#6b7280;white-space:nowrap;min-width:72px}
    .kv-val{font-weight:600;color:#111827}
    table{width:100%;border-collapse:collapse;margin-bottom:12px}
    th{padding:6px 8px;text-align:left;font-size:9px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:#fff}
    td{padding:5px 8px;border-bottom:1px solid #f3f4f6;font-size:10px;vertical-align:top}
    .right{text-align:right}
    .totals{border-radius:8px;padding:10px 12px;margin-bottom:10px}
    .tot-row{display:flex;justify-content:space-between;padding:2px 0;font-size:10px}
    .tot-grand{font-size:11px;font-weight:800;border-top:2px solid rgba(0,0,0,.15);margin-top:4px;padding-top:5px}
    .note{border-radius:6px;padding:7px 10px;font-size:9px;margin-bottom:10px;border:1px solid}
    .sig-area{display:flex;justify-content:flex-end;margin-bottom:10px}
    .sig-box{text-align:center;min-width:120px}
    .sig-img{height:40px;margin-bottom:3px;object-fit:contain}
    .sig-line{border-top:1px solid #374151;padding-top:3px;font-size:8px;font-weight:600;color:#374151}
    .footer{text-align:center;font-size:8px;color:#9ca3af;padding-top:7px;border-top:1px solid #e5e7eb}
  `;
  ...
  const buildVenueHTML = (logoB64, sigB64) => {
    ...
    return `<div class="wrap" style="font-family:'Segoe UI',Arial,sans-serif">
      ...
      <div class="hdr">
        ...
        <div class="inv-right">
          <div class="inv-title" style="color:#F59E0B">VENUE QUOTATION</div>
      ...
      <div class="two-col">
        <div class="card" style="background:#fffbeb;border-color:#fde68a">
          ...
          <div class="kv"><span class="kv-label">Customer</span><span class="kv-val">${booking?.customer?.name||booking?.customerDetails?.name||'N/A'}</span></div>
          <div class="kv"><span class="kv-label">Email</span><span class="kv-val">${booking?.customer?.email||booking?.customerDetails?.email||'N/A'}</span></div>
          <div class="kv"><span class="kv-label">Phone</span><span class="kv-val">${booking?.customerDetails?.phone||booking?.customer?.phone||'N/A'}</span></div>
          ${booking?.customerDetails?.eventType?`<div class="kv"><span class="kv-label">Event Type</span><span class="kv-val">${booking.customerDetails.eventType}</span></div>`:''}
          ${booking?.customerDetails?.guestCount?`<div class="kv"><span class="kv-label">Guests</span><span class="kv-val">${booking.customerDetails.guestCount}</span></div>`:''}
        </div>
        <div class="card" style="background:#fffbeb;border-color:#fde68a">
          <div class="card-title" style="color:#92400e;border-color:#fde68a">Supplier (Venue)</div>
          <div class="kv"><span class="kv-label">Venue</span><span class="kv-val">${booking?.venue?.businessName||'N/A'}</span></div>
          <div class="kv"><span class="kv-label">Location</span><span class="kv-val">${booking?.venue?.location?.city||''}${booking?.venue?.location?.area?', '+booking.venue.location.area:''}</span></div>
          <div class="kv"><span class="kv-label">GSTIN</span><span class="kv-val">${booking?.venue?.documents?.gstNumber||'Not Available'}</span></div>
          <div class="kv"><span class="kv-label">HSN Code</span><span class="kv-val">9973</span></div>
          <div class="kv"><span class="kv-label">Booking Date</span><span class="kv-val">${fmtDate(booking?.bookingDate)}</span></div>
          <div class="kv"><span class="kv-label">Time Slot</span><span class="kv-val">${booking?.startTime||''} – ${booking?.endTime||''} (${booking?.bookingType||''})</span></div>
        </div>
      </div>
      ...
      <div class="totals" style="background:#fffbeb;border:1px solid #fde68a">
        ...
        <div class="tot-row tot-grand" style="color:#F59E0B"><span>VENUE QUOTATION TOTAL</span><span>₹${fmt(venueTotal)}</span></div>
      </div>
  ...
  const buildPlatformHTML = (logoB64, sigB64) => `<div class="wrap" style="font-family:'Segoe UI',Arial,sans-serif">
    ...
    <div class="hdr">
      <div>
        ...
        <div class="brand-sub">Platform Service Quotation</div>
        <div style="font-size:9px;color:#6b7280;margin-top:2px">support@rentalmeet.com</div>
      </div>
      <div class="inv-right">
        <div class="inv-title" style="color:#0ea5e9">PLATFORM QUOTATION</div>
    ...
    <div class="two-col">
      ...
      <div class="card" style="background:#f0f9ff;border-color:#bae6fd">
        ...
        <div class="kv"><span class="kv-label">Company</span><span class="kv-val">RentalMeet Technologies</span></div>
        <div class="kv"><span class="kv-label">GSTIN</span><span class="kv-val">Not Available</span></div>
        <div class="kv"><span class="kv-label">HSN Code</span><span class="kv-val">999799</span></div>
        <div class="kv"><span class="kv-label">Ref Booking</span><span class="kv-val">${bookingRef}</span></div>
        <div class="kv"><span class="kv-label">Venue</span><span class="kv-val">${booking?.venue?.businessName||'N/A'}</span></div>
      </div>
    </div>
    ...
    <div class="totals" style="background:#f0f9ff;border:1px solid #bae6fd">
      ...
      <div class="tot-row tot-grand" style="color:#0ea5e9"><span>PLATFORM QUOTATION TOTAL</span><span>₹${fmt(pfTotal)}</span></div>
    </div>

    <div class="note" style="background:#fefce8;border-color:#fde68a;color:#713f12">
      <strong>Note:</strong> This invoice covers RentalMeet platform service charges only. Venue rental and GST are billed separately in the Venue Quotation.
    </div>

    ${sigB64?`<div class="sig-area"><div class="sig-box"><img src="${sigB64}" class="sig-img"/><div class="sig-line">Authorized Signatory</div></div></div>`:''}

    <div class="footer">
      <p><strong>RentalMeet</strong> — Platform Service Quotation &nbsp;|&nbsp; support@rentalmeet.com</p>
      <p style="margin-top:2px">Booking Ref: ${bookingRef} &nbsp;|&nbsp; Generated: ${fmtDT(new Date())} &nbsp;|&nbsp; Computer-generated invoice</p>
    </div>
  </div>`;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <button onClick={() => generatePDF('venue')} disabled={loading.venue}
        ...
        {loading.venue ? 'Generating...' : 'Venue Quotation'}
      </button>
      <button onClick={() => generatePDF('platform')} disabled={loading.platform}
        ...
        <FileText className="w-3.5 h-3.5" />
        {loading.platform ? 'Generating...' : 'Platform Quotation'}
      </button>
    </div>
  );
}
