    ...
    if (activeTab === 'expenses') {
      ...
      try {
        ...
        allPaidPayments.forEach((item, index) => {
          const isVenue = item.bookingType === 'venue';
          ...
          const customerName = item.customer?.name || item.customerInfo?.name || 'N/A';
          const customerEmail = item.customer?.email || item.customerInfo?.email || 'N/A';
          
          const businessName = isVenue 
            ? (item.venue?.businessName || 'N/A')
            : (item.service?.title || item.serviceSnapshot?.title || 'N/A');
          
          const location = isVenue 
            ? (item.venue?.location?.city || 'N/A')
            : (item.service?.city || item.serviceSnapshot?.city || 'N/A');
          
          rows.push([
            index + 1,
            isVenue ? 'Venue' : 'Service',
            item.bookingNumber || 'N/A',
            item.bookingDate || item.eventDate ? new Date(item.bookingDate || item.eventDate).toLocaleDateString('en-IN') : 'N/A',
            customerName,
            customerEmail,
            businessName,
            location,
